/**********************************************************************
 ...
 Type >?; for the following help:
    ...
    Commands available:
			...
			>TCXO; => Read/set TCXO Warmup time configured (ms)
...
void process_cmd(String cmd) {
  ...
  if (cmd.startsWith("?;")) {
    ...
    String ID_input = cmd.substring(cmd.indexOf('=') + 1, cmd.indexOf(';')); // Extract the message content
    traceOutput.print("Command : Set ID ");
    traceOutput.println(ID_input);
    smd_set_id(ID_input);
  } else if (cmd.startsWith("TCXO;")) {
    traceOutput.print("Command : smd read TCXO warmup time");
    smd_read_tcxo();     
  } else if (cmd.startsWith("TCXO=")) {
    String TCXO_input = cmd.substring(cmd.indexOf('=') + 1, cmd.indexOf(';')); // Extract the message content
    traceOutput.print("Command : Set TCXO (ms) ");
    traceOutput.println(TCXO_input);
    smd_set_tcxo(TCXO_input);
    ...
    String ADDR_input = cmd.substring(cmd.indexOf('=') + 1, cmd.indexOf(';')); // Extract the message content
    traceOutput.print("Command : Set ADDR ");
    traceOutput.println(ADDR_input);
    smd_set_address(ADDR_input);
    ...
    String CWParams = cmd.substring(cmd.indexOf('=') + 1, cmd.indexOf(';')); // Extract the message content
    traceOutput.print("Command : CW forward to SMD ");
    traceOutput.println(CWParams);
    smd_cw(CWParams);
  }else if (cmd.startsWith("LPM=")) {
    String LPM_Mode = cmd.substring(cmd.indexOf('=') + 1, cmd.indexOf(';')); // Extract the message content
    traceOutput.print("Command : Set LPM mode ");
    traceOutput.println(LPM_Mode);
    smd_set_lpm(LPM_Mode);
 ...
 * It also updates the LED color to indicate the operation and logs the command being sent.
 */
void smd_set_seckey(String SECKEY_input) {
  onePixel.setPixelColor(0, 255, 0, 255);  // Red = 255, Green = 0, Blue = 255 purple
  onePixel.show();
 ...
 * It also updates the LED color to indicate the operation and logs the command being sent.
 */
void smd_set_id(String ID_input) {
  onePixel.setPixelColor(0, 255, 0, 255);  // Red = 255, Green = 0, Blue = 255 purple
  onePixel.show();
...
/**
 * @brief Reads the TCXO warmup time (ms)
 * 
 * This function sends an "AT+TCXO_WU=?" command to the SMD module via the SerialSTM interface.
 * It also updates the LED color to indicate the operation and logs the command being sent.
 */
void smd_read_tcxo() {
  onePixel.setPixelColor(0, 255, 0, 255);  // Red = 255, Green = 0, Blue = 255 purple
  ...
  String test_cmd = "AT+TCXO_WU=?";
  traceOutput.print("Read smd TCXO warmup time");
  traceOutput.print(test_cmd);
  startComm = millis();
...
/**
 * @brief Write the tcxo warmup tiume in ms
 * 
 * This function sends an "AT+TCXO_WU=XXXXXX" command to the SMD module via the SerialSTM interface.
 * It also updates the LED color to indicate the operation and logs the command being sent.
 */
void smd_set_tcxo(String TCXO_input) {
  onePixel.setPixelColor(0, 255, 0, 255);  // Red = 255, Green = 0, Blue = 255 purple
  onePixel.show();

  String test_cmd = "AT+TCXO_WU=" + TCXO_input;
  traceOutput.print("Set smd TCXO Warmup time (ms)");
  traceOutput.print(test_cmd);
  startComm = millis();
 ...
 * It also updates the LED color to indicate the operation and logs the command being sent.
 */
void smd_set_lpm(String LPM_Mode) {
  onePixel.setPixelColor(0, 255, 0, 255);  // Red = 255, Green = 0, Blue = 255 purple
  onePixel.show();
...
void print_help() {
  ...
  traceOutput.println("\t - >TCXO; => Read TCXO Warmup time configured (ms)");
  traceOutput.println("\t - >TCXO=XXXXXX; => Write TCXO warmup time (ms max 30s)");
